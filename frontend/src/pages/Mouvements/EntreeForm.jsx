...
import { Package, ArrowDownToLine, Check, ChevronRight, AlertCircle, CheckCircle, Search } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import api from '../../lib/axios';

const steps = ['Sélection Article', 'Détails Entrée', 'Confirmation'];
...
export default function EntreeForm() {
  ...
  const selectArticle = (article) => {
    setForm((prev) => ({ ...prev, article_id: article.id, article }));
    setStep(1);
  };
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-24 space-y-4">
        ...
        <h2 className="text-2xl font-bold text-gray-900">Entrée enregistrée !</h2>
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Enregistrez un approvisionnement ou un retour en stock.</p>
      </div>
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {articles.map((article) => (
              <button
                ...
                className="w-full flex items-center justify-between p-4 rounded-xl border border-gray-100 hover:border-primary hover:bg-blue-50 transition-all text-left group"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-gray-900 group-hover:text-primary">{article.designation}</p>
                    <p className="text-xs text-gray-500 font-mono">{article.code}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-gray-900">{article.stock_actuel}</p>
      ...
      {step === 1 && (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
          ...
          <div className="p-4 bg-green-50 border border-green-200 rounded-xl flex items-center justify-between">
            ...
            <div className="text-right">
              <p className="text-2xl font-bold text-green-600">+{form.quantite}</p>
              <p className="text-xs text-gray-500">nouvelles unités</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              >
                ...
                <option>Transfert entrant</option>
                <option>Correction inventaire</option>
                <option>Don / Donation</option>
              </select>
            </div>
            <div>
          ...
          <div className="flex justify-between pt-4 border-t border-gray-100">
            ...
            <button onClick={() => setStep(2)} className="px-6 py-2 bg-primary text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition-colors flex items-center gap-2">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
          ...
          <div className="space-y-3">
            {[
              ...
              { label: 'Quantité entrée', value: `+${form.quantite} ${form.article?.unite || 'pcs'}` },
              { label: 'Stock après', value: `${(form.article?.stock_actuel || 0) + form.quantite} unités` },
          ...
          <div className="flex justify-between pt-4">
            ...
            <button
              ...
              disabled={saving}
              className="px-6 py-2 bg-green-600 text-white rounded-xl text-sm font-medium hover:bg-green-700 transition-colors flex items-center gap-2 disabled:opacity-60"
            >
              {saving ? 'Enregistrement...' : (<><Check className="w-4 h-4" /> Valider l'entrée</>)}
            </button>
          </div>
        </div>
      )}
